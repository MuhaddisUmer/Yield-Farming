...
async function main() {
    ...
    const MyFarm = await MyFarm.deploy(
    ...
    console.log(`MyFarm address: ${MyFarm.address}`)
    console.log(`NFT Price: ${ethers.utils.formatEther(nftPrice)} PMKN`)

    const pmknMinter = await pmknToken.MINTER_ROLE()
    await pmknToken.grantRole(pmknMinter, MyFarm.address)
    console.log(`PmknToken minter role transferred to: ${MyFarm.address}`)

    const jackMinter = await jackOLantern.MINTER_ROLE()
    await jackOLantern.grantRole(jackMinter, MyFarm.address)
    console.log(`Jack-O-Lantern NFT minter role transferred to ${MyFarm.address}`)
}
