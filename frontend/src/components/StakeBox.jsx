...
export default function StakeBox() {
    ...
    const stake = async() => {
        try {
            ...
            let tx = await daiContract.connect(signer).approve(MyFarmContract.address, amount)
            provider.waitForTransaction(tx.hash)
            .then(async() => {
                tx = await MyFarmContract.connect(signer).stake(amount)
            })
            return tx
        } catch (error) {
            alert(error)
    ...
    const unstake = async() => {
        try {
            let signer = provider.getSigner()
            let amount = ethers.utils.parseEther(transferAmount)
            let tx = await MyFarmContract.connect(signer).unstake(amount)
            return tx
        } catch (error) {
            alert(error)
        }
