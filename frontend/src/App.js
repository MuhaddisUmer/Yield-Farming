...
import MyFarm from "./abis/MyFarm.json"
...
function App() {
   ...
   */
    ...
    const [MyFarmContract, setMyFarmContract] = useState({})
    ...
    const loadMyFarmContract = useCallback(async(_provider) => {
        let MyFarmAddress = MyFarm["networks"]["42"]["address"]
    ...
    const loadStakingBalance = useCallback(async(user) => {
        let balance = await MyFarmContract.stakingBalance(user)
        setStakingBalance(balance.toString())
    }, [setStakingBalance, MyFarmContract])

    const loadPmknYield = useCallback(async(user) => {
        let balance = await MyFarmContract.calculateYieldTotal(user)
        setPmknYield(balance.toString())
    }, [setPmknYield, MyFarmContract])

    const loadPmknUnrealizedYield = useCallback(async(user) => {
        let balance = await MyFarmContract.pmknBalance(user)
        setPmknUnrealizedYield(balance.toString())
    }, [setPmknUnrealizedYield, MyFarmContract])
    ...
    */
    ...
    useEffect(() => {
        if(userAddress !== ""){
        /**
         * @notice MyFarm Events
         */
            MyFarmContract.on("Stake", async(userAddress) => {
                await loadDaiBalance(userAddress)
                await loadStakingBalance(userAddress)
            });

            MyFarmContract.on("Unstake", async(userAddress) => {
                await loadDaiBalance(userAddress)
                await loadStakingBalance(userAddress)
            })

            MyFarmContract.on("YieldWithdraw", async(userAddress) => {
                await loadPmknUnrealizedYield(userAddress)
                await loadPmknYield(userAddress)
                await loadPmknBalance(userAddress)
            })

            MyFarmContract.on("MintNFT", async(userAddress) => {
                await loadPmknBalance(userAddress)
            })
